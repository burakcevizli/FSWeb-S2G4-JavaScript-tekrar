...
function CemberinAlani(r2, piparametresi) {
	return piparametresi * (Math.pow(r2, 2))
}
...
var ucetambolunenler, enkucuk, enbuyuk, ucebolunenlerintoplami, besyuzdenkucuksayilar, siralisayilar, tekraredensayilar ,bulunanSayilar,siralisayilarsongorev;
...
tekraredensayilar = [];
bulunanSayilar = [];
siralisayilarsongorev = sayilar.sort((a,b)=>a-b)


for (let i = 0; i < siralisayilarsongorev.length; i++) {
	const arananSayi = siralisayilarsongorev[i];
	let sayiAdedi = 0;

	if (bulunanSayilar.indexOf(arananSayi) === -1) {
		for (let y = 0; y < siralisayilarsongorev.length; y++) {
			const karsilastirmaSayisi = siralisayilarsongorev[y];

			if (arananSayi === karsilastirmaSayisi) {
				sayiAdedi++;
			}
		}
	}
	if (sayiAdedi > 1) {
		tekraredensayilar.push(
			`${arananSayi }, sayısı ${ sayiAdedi } kere tekrar edilmiştir`
		);
		bulunanSayilar.push(arananSayi);
	}
}
